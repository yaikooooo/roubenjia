...
echo Starting Lottery System (Desktop Window)...
echo.
...
if /i "!choice!"=="Y" (
    ...
    echo.
    echo Installation complete! Please restart start-desktop.bat
    pause
    exit /b 0
) else (
    echo.
...
:install_deps
if exist "node_modules\.bin\electron.cmd" goto start_desktop
...
if errorlevel 1 (
    ...
    echo Then run start-desktop.bat again
    pause
    exit /b 1
)
...
start "" /min cmd /c "cd /d ""%~dp0"" && npm run desktop:frameless"
